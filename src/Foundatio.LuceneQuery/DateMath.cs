...
public static class DateMath
{
    ...
    // Supports both Elasticsearch standard (2024-01-01||+1M/d) and simplified format (2024-01-01+1M/d)
    // Operations pattern: [+\-/]\d*[yMwdhHms] - must end with a time unit letter to distinguish from timezone
    ...
    private static bool TryParseFallbackDateCore(string expression, bool isUpperLimit, out DateTimeOffset result, Func<DateTime, TimeSpan> offsetResolver)
    {
        ...
        if (_offsetRegex.IsMatch(expression) && DateTimeOffset.TryParse(expression, out DateTimeOffset explicitDate))
        {
            ...
            if (result.TimeOfDay == TimeSpan.Zero && isUpperLimit)
            {
                ...
                result = new DateTimeOffset(result.Year, result.Month, result.Day, 23, 59, 59, 999, result.Offset).AddTicks(9999);
            }
    ...
    public static DateTimeOffset RoundToUnit(DateTimeOffset dateTime, string unit, bool isUpperLimit = false)
    {
        return unit switch
        {
            "y" => isUpperLimit ? EndOfYear(dateTime) : StartOfYear(dateTime),
            "M" => isUpperLimit ? EndOfMonth(dateTime) : StartOfMonth(dateTime),
            "w" => isUpperLimit ? EndOfWeek(dateTime) : StartOfWeek(dateTime),
            "d" => isUpperLimit ? EndOfDay(dateTime) : StartOfDay(dateTime),
            "h" or "H" => isUpperLimit ? EndOfHour(dateTime) : StartOfHour(dateTime),
            "m" => isUpperLimit ? EndOfMinute(dateTime) : StartOfMinute(dateTime),
            "s" => isUpperLimit ? EndOfSecond(dateTime) : StartOfSecond(dateTime),
            _ => throw new ArgumentException($"Invalid time unit for rounding: {unit}")
        };
    }

    #region Date/Time Rounding Helpers

    private static DateTimeOffset StartOfYear(DateTimeOffset dt) =>
        new(dt.Year, 1, 1, 0, 0, 0, 0, dt.Offset);

    private static DateTimeOffset EndOfYear(DateTimeOffset dt) =>
        new DateTimeOffset(dt.Year, 12, 31, 23, 59, 59, 999, dt.Offset).AddTicks(9999);

    private static DateTimeOffset StartOfMonth(DateTimeOffset dt) =>
        new(dt.Year, dt.Month, 1, 0, 0, 0, 0, dt.Offset);

    private static DateTimeOffset EndOfMonth(DateTimeOffset dt)
    {
        int daysInMonth = DateTime.DaysInMonth(dt.Year, dt.Month);
        return new DateTimeOffset(dt.Year, dt.Month, daysInMonth, 23, 59, 59, 999, dt.Offset).AddTicks(9999);
    }

    private static DateTimeOffset StartOfWeek(DateTimeOffset dt)
    {
        // Start of week is Monday (ISO 8601)
        int diff = ((int)dt.DayOfWeek - (int)DayOfWeek.Monday + 7) % 7;
        var startOfWeek = dt.AddDays(-diff);
        return new DateTimeOffset(startOfWeek.Year, startOfWeek.Month, startOfWeek.Day, 0, 0, 0, 0, dt.Offset);
    }

    private static DateTimeOffset EndOfWeek(DateTimeOffset dt)
    {
        // End of week is Sunday (ISO 8601)
        int diff = ((int)DayOfWeek.Sunday - (int)dt.DayOfWeek + 7) % 7;
        var endOfWeek = dt.AddDays(diff);
        return new DateTimeOffset(endOfWeek.Year, endOfWeek.Month, endOfWeek.Day, 23, 59, 59, 999, dt.Offset).AddTicks(9999);
    }

    private static DateTimeOffset StartOfDay(DateTimeOffset dt) =>
        new(dt.Year, dt.Month, dt.Day, 0, 0, 0, 0, dt.Offset);

    private static DateTimeOffset EndOfDay(DateTimeOffset dt) =>
        new DateTimeOffset(dt.Year, dt.Month, dt.Day, 23, 59, 59, 999, dt.Offset).AddTicks(9999);

    private static DateTimeOffset StartOfHour(DateTimeOffset dt) =>
        new(dt.Year, dt.Month, dt.Day, dt.Hour, 0, 0, 0, dt.Offset);

    private static DateTimeOffset EndOfHour(DateTimeOffset dt) =>
        new DateTimeOffset(dt.Year, dt.Month, dt.Day, dt.Hour, 59, 59, 999, dt.Offset).AddTicks(9999);

    private static DateTimeOffset StartOfMinute(DateTimeOffset dt) =>
        new(dt.Year, dt.Month, dt.Day, dt.Hour, dt.Minute, 0, 0, dt.Offset);

    private static DateTimeOffset EndOfMinute(DateTimeOffset dt) =>
        new DateTimeOffset(dt.Year, dt.Month, dt.Day, dt.Hour, dt.Minute, 59, 999, dt.Offset).AddTicks(9999);

    private static DateTimeOffset StartOfSecond(DateTimeOffset dt) =>
        new(dt.Year, dt.Month, dt.Day, dt.Hour, dt.Minute, dt.Second, 0, dt.Offset);

    private static DateTimeOffset EndOfSecond(DateTimeOffset dt) =>
        new DateTimeOffset(dt.Year, dt.Month, dt.Day, dt.Hour, dt.Minute, dt.Second, 999, dt.Offset).AddTicks(9999);
